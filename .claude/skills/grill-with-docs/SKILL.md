...
- [ ] **Actions** — composite actions under `actions/*/action.yml`: inputs, steps, cache keys, pinned third-party actions
- [ ] **Workflows** — reusable workflows under `.github/workflows/*.yml`: `workflow_call` inputs/secrets, jobs, permissions, pinned actions
- [ ] **Consumer Impact** — SHA-pinned callers: because SHA pinning is enforced account-wide, a behavioral change here reaches consumers only when they bump their pin of this repo. Call out what changes for consumers and whether it is breaking on pin bump.
- [ ] **Docs** — `README.md` (contents table, SHA-pinning rule, usage snippets), CONTEXT.md, ADRs, any other affected docs

If an area has no changes, state that explicitly ("Consumer Impact: no change on pin bump") so it's clear it was considered, not skipped.
...
When the user states how something works, check whether the code agrees. If you find a contradiction, surface it: "Your workflow pins action X at SHA Y, but you just said the caller controls that pin — which is right?"
...
| Actions | ... | ... |
| Workflows | ... | ... |
| Consumer Impact | ... | SHA-pinned callers — behavioral changes reach consumers only on pin bump |
| Docs | ... | ... |

### Behaviors to Implement
- [ ] Behavior 1 (area: workflows)
- [ ] Behavior 2 (area: docs)
