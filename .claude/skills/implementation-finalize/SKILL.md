...
- `commit_type` (required): Type of commit (feat, fix, refactor, docs, chore)
...
## Implementation
- Changes: <what changed in actions/workflows/docs>
- Verify: <actionlint / YAML-parse result, or documented skip>
- Tests: documented skip — no test suite in this repo

Closes #<issue-number>
```

An empty `### Deviations` or `### Decisions` subsection emits `None.` under its
heading (the heading still appears). Record the verify-state outcome (actionlint,
YAML-parse fallback, or documented skip) in the `## Implementation` section so the
verify path is never silently assumed.
...
- Does NOT stage or commit — commits happen at:
  ...
  - after user approval: `feat:/fix:/refactor:/chore:` commit for the change
