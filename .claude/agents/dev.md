...
- **Resolved commands** (rendered per-repo from the distribution registry):
  - Test/red-green loop command: `actionlint .github/workflows/*.yml`
  - Validate/verify/build command: `actionlint .github/workflows/*.yml`
...
- Run with `actionlint .github/workflows/*.yml` (or the repo's resolved test command).
...
### 5. Validate
- Run the repo's validator/build: `actionlint .github/workflows/*.yml` (or the
...
- `actionlint .github/workflows/*.yml` (or resolved validator) and the full suite pass ✓
