...
```
@milestone-implementation-orchestrator https://github.com/derekwinters/chores-web-actions/milestone/N
```
...
```
Milestone: Notification System (#10)
State: [8] CI Watch
Progress: 8/9
Branch: claude/milestone-10-mzvlv8
Issues: 8/8 complete
...
- Shared action/workflow changes before consumers that reference them
- Reusable workflow inputs before callers that pass them
- Independent issues ordered arbitrarily (by issue number)

Output:
```
Implementation order:
1. #301 Add reusable notify workflow — shared infra, must precede callers
2. #303 Wire notify into docker-build-push — depends on #301
3. #302 Update README trigger table — depends on #303
```
...
`<randomid>` is 6 lowercase alphanumeric characters (e.g. `claude/milestone-10-mzvlv8`). This branch is cut fresh from `main` and its lifecycle (creation, the setup commit, and every push) is owned exclusively by this orchestrator — see Branch Ownership below.
...
Written **in full at draft-pr creation time** (start of work, before any issue is implemented), so the PR always shows complete vs. incomplete issues at a glance.
...
- actionlint failures in `.github/workflows/*.yml` introduced by this milestone
- Invalid YAML or missing required keys in `actions/*/action.yml` introduced by this milestone
- Third-party action pin/reference mistakes introduced by this milestone
...
- Failures in workflow/action code predating this milestone (not caused by these changes)
